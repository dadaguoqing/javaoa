	...
	<div style="margin-top:10px;position:relative; line-height:20px; height:22px; border-bottom:1px solid #ccc;">
		...
		<table class="lzzUIGrid"><tbody>
			...
			<tr>
				...
				<td class="l-td-header" style="width:120px;">入职日期</td>
	    	...
	    	<c:forEach items="${list}" var="cur" >
	    		<tr>
	    			...
	    			<td class="l-td">${cur.entryDate }</td>
	    			<td class="l-td l-td-left">${cur.deptName }</td>
	    			<td class="l-td l-td-left">${cur.phone1 }</td>
	    			<td class="l-td">
						<a href="${ctx}/web/oa/user/edit?id=${cur.id}">详情</a> | <a href="${ctx}/web/oa/role/emp?empId=${cur.id}">设置角色</a>
						
						| <a href="javascript:delUser(${cur.id},'${cur.name }')">删除</a>
					</td>
	    		</tr>
	    	</c:forEach>

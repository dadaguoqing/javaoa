...
<body onbeforeunload="setUids()" style="background-color:#efe; padding:10px 0 10px 10px; overflow:scroll; overflow-x:hidden;">
	...
	<label for="qx">全选</label> <input id="qx" checked="checked" type="checkbox"/>
	<!--  
	...
	<table class="table1" ><tbody>
		<c:forEach items="${duMap[cur.id]}" var="c" varStatus="st">
    		...
    		<td class="l-td" style="padding-top:10px; width:100px; padding-bottom:10px;">
    			<label for="cb${c.id}">${c.name }</label> <input id="cb${c.id}" type="checkbox" checked="checked" value="${c.id}" class="uids"/>
    		</td>

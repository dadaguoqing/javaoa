			...
			<div class="gridContainer"
				...
				<table class="lzzUIGrid">
					...
					<tr>
						...
						<td class="l-td-header" style="width: 100px;">请购人</td>
						<td class="l-td-header" style="width: 100px;">项目编号</td>
						<td class="l-td-header" style="width: 150px;">编号</td>
						<td class="l-td-header" style="width: 250px;">采购事由</td>
					...
					<c:forEach items="${list}" var="mp" varStatus="i">
						<tr>
							...
							<td class="l-td">${AllUsers[mp.qgId].name }</td>
							<td class="l-td limitText" title="${mp.projectCode}">${empty mp.projectCode ? '无' : mp.projectCode}</td>
							<td class="l-td">${mp.purchaseCode}</td>
							<td class="l-td limitText" title="${mp.reason}">${mp.reason}</td>
							<td class="l-td">${mp.daystr}</td>
							<td class="l-td"><a href="${ctx}/web/oa/materia_v2/dealInwareDetail?id=${mp.id}">查看详情</a></td>
						</tr>
					</c:forEach>

...
public class UploadPhotoServlet extends HttpServlet {
	...
	//支持的文件类型
	private String[] types = new String[]{".jpg",".jpeg",".png",".gif"};
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		try{
			...
			while (iter.hasNext()) {
			    ...
			    if (item.isFormField()) {//普通字段
			        ...
			        if(!isValid || !contentType.contains("image")){
			        	throw new InvalidContentTypeException();
			        }
			        
			        String savePath = "empPhoto";
			        ...
			        loginUser.setPhoto(loc);
			        
			    }
			    
			}
			
			this.userService.updatePhoto(loginUser);
			//this.noticeService.addRule(rule);
		}catch (SizeLimitExceededException e) {
			msg = "您上传的照片超过规定大小（" + uploadSize + "MB）。";
			e.printStackTrace();
		} catch (InvalidContentTypeException e) {
			msg = "您上传照片的格式不符合规定。（服务器支持的格式：.jpg,.png,.gif）";
			e.printStackTrace();
		...
		//跳转页面
		request.getRequestDispatcher("/WEB-INF/views/oa/emp/changeImg.jsp").forward(request, response);

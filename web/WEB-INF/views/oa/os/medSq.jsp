...
<div style="margin: 10px 5px 0 195px;">
	...
	<div class="lzui-tooltips" style="margin:10px; color:#C0392B; font-weight:bold; font-size:14px;">
  		<a style="color:#C0392B;" href="${ctx}/web/oa/os/myMedSq">点击查看申领记录</a>
  	</div>
	
	<form name="form1" action="${ctx }/web/oa/os/medSq" method="post">
...
$(function(){
	...
	$('#crbtn').click(function(){
		...
		index = $.layer({
		    ...
		    border: [3],
		    shadeClose: false,
		    iframe: {src : '${ctx}/web/oa/os/chooseMed'},
		    area: ['850px', '500px'],
...
function addcd(){
	...
	var html = '<tr id="cd'+count+'">'+
		...
		'<td style=" border-right:none; ">'+
			'<a href="javascript:delcd('+count+', '+ thId+ ');">删除</a>'+
		'</td>'+
		'</tr>';
	...
	/*
	$.post('${ctx}/web/oa/os/ajaxGetMed',{id:thId},function(data){
		$('#cd'+count).after(data);
	});*/
}

function ajaxAddBz(id){

}

function delcd(tid, thId){
	$('#cd'+tid).remove();
	$('#tr'+thId).remove();
}

...
<div style="margin-top:10px;position:relative; line-height:20px; height:22px; border-bottom:1px solid #ccc;">
	...
	<div style="width:92px; color:#34495E;border-bottom:2px solid #27AE60; padding-left:10px; font-size:14px; font-weight:bold;">员工部门管理</div>
	<form name="form1" method="post" action="${ctx}/web/oa/user/setUserDept">
	<div class="lzui-tooltips" style="padding:10px; color:#C0392B; font-weight:bold; font-size:15px;">
  				...
  				<div class="">
					...
					<div class="lzui-headers">
						<div class="t1"><span>1</span>第一步、选择一个部门</div>
						...
						<div class="lzui-table-wapper" style="margin-top:7px;">
			  			...
			  			<tr class="lzui-td rtd">
			  				<td style="text-align:center; padding-left:15px;"><input onclick="ajaxEmps();" class="rids" name="deptId" type="radio" value="${cur.id }"/></td>
			  				<td>${cur.name }</td>
			  			</tr>
			  			</c:forEach>
					    </table>
					...
					<div class="lzui-headers">
						<div class="t2"><span>2</span>第二步、选择该部门的所有员工</div>
						
						<div class="lzui-table-wapper" style="margin-top:7px; height:600px; overflow:scroll;">
			  			<table class="lzui-table" style="width:100%;">
			  			<tr>
				  		...
				  		<tr class="lzui-td utd" uid="${cur.id }">
					    	<td style=" text-align:center;"><input type="checkbox" class="ids uids" name="empIds" value="${cur.id }" id="u${cur.id}"/></td>
					    	<td>${cur.name }</td>
					    	<td>${cur.code }</td>
					    </tr>
					    </c:forEach>
...
function ajaxEmps(){
	...
	$.get('${ctx}/web/oa/role/ajaxEmpsByDept/'+rid+"?t="+Math.random(), function(data){

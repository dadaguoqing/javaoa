...
<div
	...
	<div style="margin: 10px 5px 0 195px;">
		...
		<div class="lzui-tooltips"
			...
			<a style="color: #C0392B;" href="${ctx}/web/oa/external/apply">返回</a>
		</div>
			...
			<table class="lzzUIGrid">
				<tbody>
					...
					<tr>
						...
						<td class="l-td-header" style="">状态</td>
					...
					<c:forEach items="${list}" var="cur">
						<tr>
							...
							<td class="l-td" style="text-align:left;">
								...
								<c:if test="${cur.status == 6}">
									生产主管审批中
								</c:if>
								<c:if test="${cur.status == 7}">
									生产总监审批中
								</c:if>
								<c:if test="${cur.status == 8}">
									财务会计审批中
								</c:if>
								<c:if test="${cur.status == 9}">
									总经理审批中
								</c:if>
								<c:if test="${cur.status == 10}">
									生产加工中、待入库
								</c:if>
								<c:if test="${cur.status == 11}">
									完成入库
								</c:if>
								<c:if test="${cur.status == -1}">
									审批不通过
							...
							<td class="l-td">
								<a href="${ctx}/web/oa/external/detail/apply?id=${cur.id}">查看详情</a>
							</td>
						</tr>
					</c:forEach>
...
<script>
	onload = function() {
	    //用用回调
	    function go(p) {
	    	location.href = '${ctx}/web/oa/external/applyList?curPage=' + p;
	    }

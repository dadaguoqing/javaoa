...
<div
	...
	<div style="margin: 10px 5px 0 195px;">
		...
		<div class="lzui-tooltips"
			...
			<a style="color: #C0392B;" href="${ctx}/web/oa/merchandise/apply/recordList/2/1?curPage=1">查看申请记录</a>
		</div>

		<form name="form1" action="${ctx }/web/oa/merchandise/apply/saveMerOutbound" id="form1"
			method="post" >
			<input type="hidden" value="${submitCode}" name="submitCode" /> 
			<input type="hidden" id="number1" name="number" />
			<div style="border: 1px solid #2191C0;">
				<div class="main-header" id="id1">
					<span style="color: #eee;">${compName }成品出库申请单</span>
				</div>
				...
				<table class="table1" id='cdTable'>
					<tbody id="data">
						...
						<tr class="data">
							...
							<td align="center" >
								<input  type="number" value="1" id="quantity" style="width: 50px" min="1" onchange="testnum(this)" >
							</td>
							<td align="center">
							...
							<td align="center">
								<select name="" id="inboundReason">
									<option value="-1">=请选择出库原因=</option>
								</select>
							</td>
							<td align="center">
								<input name="" id="packageCode" style="width: 120px">
							</td>
							<td align="center">
...
<script>
	...
	function getInboundReasons() {
		$.get("getInboundReasons",{type : 2},function(result) {
			// 部署删除
			if (mode == 1) {
				result = JSON.parse(result);
			}
			var option = '';
			$.each(result,function(i,obj){
				option += '<option value="'+  obj.content +'">'+ obj.content +'</option>';
			});
			$('#inboundReason').append(option);
		});
	}
	...
	function add(obj) {
		...
		var inboundSource = $('#inboundReason').val();
		if (inboundSource == -1) {
			layer.msg('请选择出库原因！');
			return;
		}
		...
		tr += '<td class="limitText" title="'+inboundSource+'">'
				+ inboundSource
				+ '<input name="inboundReason" value="'+inboundSource+'" type="hidden" /></td>';
		tr += '<td class="limitText" title="'+packageCode+'">'
				+ packageCode
				+ '<input name="packageCode" value="'+packageCode+'" type="hidden" /></td>';

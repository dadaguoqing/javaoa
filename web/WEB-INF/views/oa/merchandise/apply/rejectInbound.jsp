...
<div
	...
	<div style="margin: 10px 5px 0 195px;">
		...
		<div class="lzui-tooltips"
			...
			<a style="color: #C0392B;" href="${ctx}/web/oa/merchandise/apply/recordList/5/1?curPage=1">查看申请记录</a>
		</div>

		<form name="form1" action="${ctx }/web/oa/merchandise/apply/saveRejectInbound" id="form1"
			method="post" >
			<input type="hidden" value="${submitCode}" name="submitCode" /> 
			<input type="hidden" id="number1" name="number" />
			<div style="border: 1px solid #2191C0;">
				<div class="main-header" id="id1">
					<span style="color: #eee;">${compName }不良品入库申请单</span>
				</div>
				...
				<table class="table1">
					<tbody>
						<tr>
							...
							<td id="numberStr"><input type="hidden" id="applyCode" name="applyCode"></td>
						</tr>
					</tbody>
				</table>
				<div class="tableTitle" style="padding: 10px 20px;">
				...
				<table class="table1" id='cdTable'>
					<tbody id="data">
						<tr>
							...
							<td class="title" style="text-align: center; width: 80px;">入库原因</td>
...
<script>
	...
	function getInboundReasons() {
		$.get("getInboundReasons", {type : 1}, function(result) {
			// 部署删除
			if (mode == 1) {
				result = JSON.parse(result);
			}
	...
	function add(obj) {
		...
		var inboundReason = $('#inboundReason').val();
		if (inboundReason == -1) {
			layer.msg('请选择入库原因！');
			return;
		}
		...
		tr += '<td class="limitText" title="'+inboundReason+'">'
				+ inboundReason
				+ '<input name="inboundReason" value="'+inboundReason+'" type="hidden" /></td>';

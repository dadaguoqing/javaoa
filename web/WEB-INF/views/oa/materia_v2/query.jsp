...
<div
	...
	<div style="margin: 10px 5px 0 195px; padding-bottom: 20px;">
		...
		<form name="sForm" id="sForm" action="${ctx}/web/oa/materia_v2/queryAjax"
			method="post">
			...
			<table id="baseInfoTable">
				<tr>
					...
					<td><input type="text" name="materiaCode" id="materiaCode"
						placeholder="物料编码" class="form-controller" value="${materiaCode}"></td>
					<td><input type="text" name="spec" id="spec" value="${spec}"
						placeholder="规格型号" class="form-controller"></td>
					<td><input type="text" name="all" id="all" placeholder="全部条件" value="${all}"
			...
			<table class="table1" id='cdTable'>
				<tbody>
					<tr>
						...
						<td class="l-td-header " style="width: 9%;">品名分类</td>
					...
					<c:if test="${not empty page.list}">
						<c:forEach items="${page.list}" var="list" varStatus="i">
							<tr>
								<td>${i.count}</td>
								<td>${list.materiaCode}</td>
								<td class="limitText" title="${list.spec}">${list.spec}</td>
								<td class="limitText" title="${list.classfiy}">${list.classfiy}</td>
								<td class="limitText" title="${list.brand}">${list.brand}</td>
								<td class="limitText" title="${list.package1}">${list.package1}</td>
								<td class="limitText" title="${list.warehouse}">${list.warehouse}</td>
								<td class="limitText" title="${list.stock}">${list.stock}</td>
								<td >
									<a href="${ctx }/web/oa/materia_v2/queryMore?id=${list.id}&whId=${list.warehouseId}" target="_blank">
									查看详情</a></td>
							</tr>
						</c:forEach>
					</c:if>
...
<script>
	...
	onload = function() {
	    //用用回调
	    function go(p) {
	    	sForm.action = '${ctx}/web/oa/materia_v2/queryAjax?curPage=' + p;
	    	$('#sForm').submit();
	    }

		...
		<div class="lzui-tooltips"
			style="margin: 10px; color: #C0392B; font-weight: bold; font-size: 14px;">
					<a style="color: #C0392B;"
						href="${ctx}/web/oa/materia_v2/reject">返回申请</a>
		</div>
					...
					<c:if test="${empty ljchen }">
						<c:forEach items="${list}" var="cur">
							<tr>
								...
								<td class="l-td"><a
									href="${ctx }/web/oa/materia_v2/rejectDetail?id=${cur.id}">查看详情</a></td>
							</tr>
						</c:forEach>
					</c:if>
...
<script>
	...
	onload = function() {s
	    //用用回调
	    function go(p) {
	    	location.href = '${ctx}/web/oa/materia_v2/rejectList?curPage=' + p;

		...
		<div class="lzui-tooltips"
			style="margin: 10px; color: #C0392B; font-weight: bold; font-size: 14px;">
					<a style="color: #C0392B;"
						href="${ctx}/web/oa/materia_v2/rejectList3">查看记录</a>
		</div>
					...
					<c:if test="${empty ljchen }">
						<c:forEach items="${list}" var="cur">
							<tr>
								...
								<td class="l-td"><a
									href="${ctx }/web/oa/materia_v2/rejectDetail2?id=${cur.id}&type=1">查看详情</a></td>
							</tr>
						</c:forEach>
					</c:if>
...
<script>
	...
	onload = function() {
	    //用用回调
	    function go(p) {
	    	location.href = '${ctx}/web/oa/materia_v2/rejectApprove?curPage=' + p;

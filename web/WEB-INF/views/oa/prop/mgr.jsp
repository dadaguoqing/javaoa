...
<div style="margin: 10px 5px 0 195px; padding-bottom:20px;">
	...
	<table id="baseInfoTable">
		<tr>
			...
			<td>
			<div>
				<a href="${ctx}/web/oa/prop/exportAllRecord"  id="smbtn" class="lzui-btn lzui-corner-all" style="padding:0px 20px;">导出记录</a>
	</div>
			</td>
		</tr>
	</table>
	...
	<table class="lzzUIGrid"><tbody>
		...
		<tr>
			...
			<td class="l-td-header l-td-left" style="width:60px;">资产种类</td>
			<td class="l-td-header l-td-left" style="width:132px;">资产编码</td>
			<td class="l-td-header l-td-left" style="width:100px;">资产名称</td>
			<td class="l-td-header l-td-left" style="width:130px;">规格型号</td>
			<td class="l-td-header l-td-left" style="width:45px">负责人</td>
			<td class="l-td-header l-td-left" style="width:60px">放置地点</td>
			<!--  
			<td class="l-td-header l-td-left" style="width:70px">单价（元）</td>
			-->
			<td class="l-td-header l-td-left l-td-last-right" style="width:170px">操作</td>
		</tr>
    	<c:forEach items="${list}" var="cur" varStatus="st">
    		<tr>
				...
				<td class="l-td l-td-left">${cur.placeName}</td>
				...
				<c:if test="${loginUser.id != 1}">
					<a href="javascript:editDetail(${cur.id})">编辑</a> |
					<a href="javascript:showDetail('${cur.name }',${cur.id})">查看详情</a> |
					<a href="${ctx}/web/oa/prop/goWx?id=${cur.id}">申请维修</a> |
					<a href="${ctx}/web/oa/prop/goZx?id=${cur.id}">申请注销</a>
				</c:if>
...
function editDetail(id){
	//document.location.href="${ctx}/web/oa/prop/goEdit/"+id;

	var sf = $('#sForm');
	sf.attr('action', "${ctx}/web/oa/prop/goEdit/"+id);

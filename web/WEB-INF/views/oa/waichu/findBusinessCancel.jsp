...
<div
	...
	<div id="theNavigater">
		...
		<div class="navTitle2" url="${ctx }/web/oa/waichu/findAllCancelBusiness?curpage=1"><img src="${ctx }/resources/images/item.png" style="vertical-align:middle;width:20px;height:20px;"/>&nbsp;&nbsp;返回上级</div>
	</div>
	...
	<div style="margin: 10px 5px 0 195px;">
		...
		<div
			style="border: 1px solid #E67E22; display: none; margin-bottom: 10px;"
			id="leaveDiv">
			<div class="main-header" id="id11">
				<span style="color: #E67E22">原始注销出差申请单</span>
			</div>
			...
			<table class="table1">
				<tbody>
					<tr>
						<td class="title" style="border-left: none; width: 90px;">类型：</td>
						<td style="width: 90px;">${l.type }</td>
						<td class="title" style="width: 60px;">申请人：</td>
						<td style="width: 90px;">${l.proposerName }</td>
						<td class="title" style="border-left: none; width: 90px;">所属部门：</td>
						<td><span id="deptSpan">${AllDepts[l.deptId].name  }</span></td>
						<td class="title" style="width: 90px;">申请时间：</td>
						<td style="border-right: none; width: 200px;">${l.createTime }
						</td>
					</tr>
				</tbody>
			</table>
			...
			<table class="table1">
				<tbody>
					<tr>
						...
						<td style="width: 160px;">${l.beginTime }</td>
						<td class="title" style="width: 90px;">结束时间：</td>
						<td style="width: 160px;">${l.endTime }</td>
						<td class="title" style="width: 90px;">出差时长：</td>
						<td style="width: 160px;">${l.days }天${l.hours }小时${l.minutes }分钟
						</td>
			...
			<table class="table1">
				<tbody>
					<tr>
						<td style="border-right: none;"><textarea disabled="disabled"
								name="content"
								style="width: 50%; height: 120px; padding: 5px 10px;">${l.content }</textarea>
						</td>
					</tr>
				</tbody>
			</table>
			...
			<table class="table1">
				<tbody>
					<tr>
						<td style="width: 16%;">主管意见 <c:if
								test="${ not empty l.mgrId }">
								<span style="color: red;">（${AllUsers[l.mgrId].name}）</span>
							</c:if>
						</td>
						<td style="width: 16%; color: red;"><c:choose>
					...
					${ empty l.mgrCmt ? '暂未到达该流程' : l.mgrCmt  }
						...
						<td style="width: 16%;">总监意见 <c:if
								test="${ not empty l.directId }">
					...
					${ empty l.directCmt ? '暂未到达该流程' : l.directCmt  }
						...
						<td style="width: 16%;">总经理意见 <c:if
								test="${ not empty l.bossId }">
								<span style="color: red;">（${AllUsers[l.bossId].name}）</span>
							</c:if>
						</td>
						<td style="width: 16%; color: red; border-right: none;"><c:choose>
		...
		<form name="form1" action="" method="post" onsubmit="">
			...
			<input type="hidden" value="${submitCode}" name="submitCode" /> <input
				type="hidden" value="${leave.id}" name="id" />
			<div style="border: 1px solid #2191C0;">
				<div class="main-header" id="id1">
					<span style="color: #eee;">${compName }<span style="">注销申请单</span></span>
				</div>
				<div class="tableTitle" style="padding: 10px 20px;">
					<span> <img src="${ctx }/resources/images/communication.gif" />&nbsp;&nbsp;&nbsp;&nbsp;
						基本信息
				...
				<table class="table1">
					<tbody>
						<tr>
							<td class="title" style="border-left: none; width: 90px;">类型：</td>
							<td style="width: 90px;">${leave.type }</td>
							<td class="title" style="width: 60px;">申请人：</td>
							<td style="width: 90px;">${leave.proposerName }</td>
							<td class="title" style="border-left: none; width: 90px;">所属部门：</td>
							<td>
							...
							<td style="border-right: none; width: 200px;">
								${leave.createTime }</td>
							<td class="title" style="border-left: none; width: 80px;">注销操作人：</td>
							<td style="width: 180px; font-weight: bold;">
								${empty leave.updateName ?"未知":leave.updateName}</td>
						</tr>
					</tbody>
				</table>
				<div class="tableTitle" style="padding: 10px 20px;">
				...
				<table class="table1">
					<tbody>
						<tr>
							<td class="title" style="border-left: none; width: 90px;">开始时间：</td>
							<td style="width: 160px;">${leave.beginTime }</td>
							<td class="title" style="width: 90px;">结束时间：</td>
							<td style="width: 160px;">${leave.endTime }</td>
							<td class="title" style="width: 90px;">注销时长：</td>
							<td style="width: 160px;">${leave.days }天${leave.hours }小时${leave.minutes }分钟
							</td>
						</tr>
					</tbody>
				</table>
